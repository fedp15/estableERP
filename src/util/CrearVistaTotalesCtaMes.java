 ...
 * @author Ricardo
 */
public class CrearVistaTotalesCtaMes extends Modelos.conexion{
    ...
    public CrearVistaTotalesCtaMes() {
    ...
    public void armarVista(){
        try {
            ...
            SQL = "select comp_empresa,cuenta as cta, cta_nombre as ctaNombre, cta_nivel as ctaNivel,\n" +
                  ...
                  "		 (sum(Montos_Debe*signo)+sum(Montos_haber*signo)) as saldoMes,\n" +
                  "		 MONTH(comp_fecha) as mes,YEAR(comp_fecha) as ano, comp_fecha\n" +
                  "from asientos_contables \n" +
                  "where aprobado = 1 and doc_saldo_inicial=0\n" +
                  "group by cuenta,MONTH(comp_fecha)\n" +
                  "order by ano,mes ";
            System.out.println(SQL);
                
            String deleteView = "DROP VIEW if EXISTS totalizador_ctas_mes";
            insertarconex.SqlInsert(deleteView);
                
            String createView = "CREATE VIEW totalizador_ctas_mes AS\n"+SQL;
            insertarconex.SqlInsert(createView);
        } catch (Exception ex) {
            Logger.getLogger(CrearVistaTotalesCtaMes.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}