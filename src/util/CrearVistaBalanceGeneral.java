 ...
 * @author Ricardo
 */
public class CrearVistaBalanceGeneral extends Modelos.conexion{
    ...
    public CrearVistaBalanceGeneral() {
    ...
    public void armarVista(){
        try {
            ...
            SQL = "select comp_empresa,comp_fecha,cuenta as cta, cta_nombre as ctaNombre, cta_nivel as ctaNivel, aprobado, \n" +
                  ...
                  "where cta_nivel_1 <=3 and aprobado = 1 and doc_saldo_inicial=0\n" +
                  "group by cuenta,comp_empresa\n" +
                  "order by cuenta ";
            System.out.println(SQL);
            
                
            String deleteView = "DROP VIEW if EXISTS balance_general";
            insertarconex.SqlInsert(deleteView);
                
            String createView = "CREATE VIEW balance_general AS\n"+SQL;
            insertarconex.SqlInsert(createView);
        } catch (Exception ex) {
            Logger.getLogger(CrearVistaBalanceGeneral.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}