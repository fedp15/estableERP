...
public class PDF {
    ...
    public static void print(String nFactura, Date fecha, String empleado, List<ProductoVO> productos, BigDecimal subtotal,
                             BigDecimal mtoIva, BigDecimal totalDoc) throws IOException{
        ...
        String fileName = direccion + "/"+ dateF.format(fecha)+"_"+nFactura+".pdf";
...
//        content.showText("Telefono : " + cliente.getConvencional_C());
        ...
        for (ProductoVO p : productos){
            ...
            content.setNonStrokingColor(Color.BLACK);
            content.setFont(PDType1Font.COURIER, 9);
            content.newLineAtOffset(430, vertical);
            //content.showText("" +  p.getPrecio_venta());
            content.showText(""+df.format(p.getPrecio_venta()));
            content.endText();
            
            content.beginText();
            content.setNonStrokingColor(Color.BLACK);
            content.setFont(PDType1Font.COURIER, 9);
            content.newLineAtOffset(510, vertical);
            //content.showText("" +  p.getPrecio_venta().multiply(new BigDecimal(p.getCantidad())));
            content.showText(""+df.format(p.getPrecio_venta().multiply(new BigDecimal(p.getCantidad()))));
            content.endText();
...
//        content.endText();
        ...
        content.beginText();
        content.setNonStrokingColor(Color.BLACK);
        content.newLineAtOffset(380, 45);
        content.setFont(PDType1Font.COURIER_BOLD, 11);
        content.showText("TOTAL BASE: " + ""+df.format(subtotal.doubleValue()));
        content.newLine();
        content.showText("TOTAL IVA : " + ""+df.format(mtoIva.doubleValue()));
        content.newLine();
        content.showText("TOTAL     : " + ""+df.format(totalDoc.doubleValue()));
        content.endText();
	...
	doc.close();
        ...
        //Process p = Runtime.getRuntime().exec(new String[]{"pdf",System.getProperty("user.dir")+"\\"+fileName});
